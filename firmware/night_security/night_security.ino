...
void publishSensorData(uint32_t now,const security::Output& out,bool armed) {
    ...
    Serial.print("S,");Serial.print(sequence++);Serial.print(',');Serial.print(now);
    Serial.print(',');Serial.print(out.pir);Serial.print(',');Serial.print(out.radar);
    Serial.print(',');Serial.print(out.metres,3);Serial.print(',');Serial.print(!out.range_fault);
    Serial.print(',');Serial.print(out.alarm);Serial.print(',');Serial.println(armed);
}
void setup() {
    pinMode(PIR_PIN,INPUT);pinMode(RADAR_PIN,INPUT);pinMode(ECHO_PIN,INPUT);
    pinMode(TRIG_PIN,OUTPUT);pinMode(RESET_PIN,INPUT_PULLUP);
    for (uint8_t pin=GREEN_PIN;pin<=BUZZER_PIN;++pin) { pinMode(pin,OUTPUT);digitalWrite(pin,LOW); }
    Serial.begin(115200);startMs=millis(); // Never wait indefinitely for Serial.
#if defined(ARDUINO_ARCH_RENESAS_UNO)
    WDT.begin(4000);
#endif
}
void loop() {
    const uint32_t now=millis();processServerMessage(now);
    if (security::elapsed(now,lastSample)>=100U) {
        lastSample=now;
        const bool armed=security::elapsed(now,startMs)>=60000U;
        if (digitalRead(RESET_PIN)==LOW) { controller.reset(); }
        const auto out=controller.tick(now,digitalRead(PIR_PIN)==HIGH,
                                       digitalRead(RADAR_PIN)==HIGH,readDistance(),armed);
        updateAlarmOutputs(out,armed);publishSensorData(now,out,armed);
    }
#if defined(ARDUINO_ARCH_RENESAS_UNO)
    WDT.refresh();
#endif
