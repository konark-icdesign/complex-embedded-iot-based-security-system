...
class Regression(unittest.TestCase):
    def test_missing_download_is_reported_as_not_run(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "fixtures" / "esc50"
            p.mkdir(parents=True)
            (p / "manifest.json").write_text(
                json.dumps([{"filename": "missing.wav", "status": "downloaded"}])
            )
    ...
    def test_global_light_and_local_change(self):
        x = room()
        corrected, _, _ = compensate(x, x + 60)
        self.assertLess(np.mean(np.abs(corrected) > 12), 0.001)
        y = x.copy()
        y[30:80, 40:70] += 60
    ...
    def test_saturated_camera_not_motion(self):
        c = Camera()
        c.update(0, room().astype(np.uint8))
        v = c.update(0.2, np.full((120, 160), 255, dtype=np.uint8))
    ...
    def test_median_glitch_and_timeout(self):
        s = Sensors()
        for i in range(20):
            self.assertFalse(s.update(False, False, 0.35 if i == 9 else 3.0)[2])
        for _ in range(12):
    ...
    def test_durable_retry_lost_ack(self):
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "outbox.db"
            o = Outbox(p)
            o.enqueue(dict(id="persistent-id", t=8.0))
            o.pump(False)
    ...
    def test_packet_freshness(self):
        p = PacketGate()
        self.assertTrue(p.accept(0, 1.0, 1.1))
        self.assertFalse(p.accept(0, 1.0, 1.2))
        self.assertFalse(p.accept(1, 1.0, 4.0))
        self.assertFalse(p.accept(1, 4.0, 3.0))
        self.assertTrue(p.accept(2, 4.0, 4.1))

    def test_old_packet_cannot_refresh_physical_evidence(self):
        f = Fusion()
        f.update(1.0, dict(A=True, P=True, M=True))
        f.update(10.0, dict(P=True, M=True), captured=dict(P=1.0, M=1.0))
        f.update(10.1, dict(A=True))
        self.assertNotEqual(f.state, 2)

    def test_acquisition_timestamps_allow_bounded_delay(self):
        f = Fusion()
        f.update(8.2, dict(A=True), captured=dict(A=8.0))
        f.update(8.7, dict(P=True), captured=dict(P=8.5))
        state, _, _, _ = f.update(9.2, dict(M=True), captured=dict(M=9.0))
        self.assertEqual(state, 2)

    def test_persistent_camera_fault_does_not_merge_separate_incidents(self):
        f = Fusion()
        e = dict(P=True, M=True, U=True)
        f.update(1.0, e, True)
        f.update(1.1, e, True)
        f.update(10.0, {}, True)
        self.assertEqual(f.state, 1)
        f.update(12.0, e, True)
