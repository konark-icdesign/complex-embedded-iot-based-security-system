...
class AudioChecks(unittest.TestCase):
    ...
    def test_silence_and_clipping_are_invalid(self):
        ...
        self.assertFalse(features(np.tile([-1.,1.],2048))['valid'].any())
    ...
    def test_invalid_window_cannot_inherit_anomaly(self):
        model=Baseline(np.zeros(9),np.ones(9),threshold=.5)
        _,flags=model.detect({'x':np.ones((3,9)), 'valid':np.array([True,True,False])})
