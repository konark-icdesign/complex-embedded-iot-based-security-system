...
def run():
    ...
    train = np.concatenate(
        [
            features(background(12, s, gain=g))["x"]
            for s, g in [(11, 0.5), (12, 1), (13, 2), (14, 1.4)]
        ]
    )
    calibration = np.concatenate(
        [features(background(12, s, gain=g))["x"] for s, g in [(101, 0.7), (102, 1.7)]]
    )
    ...
    for seed in range(1000, 1010):
        ...
        variants = {
            "pc_background": normal,
            "lower_constant_gain": normal * 0.25,
        ...
        for name, wave in variants.items():
            ...
            rows.append(
                dict(
                    ...
                    invalid_windows=int(np.sum(~f["valid"])),
                    danger_alert=False,
                )
            )
    out = ROOT / "results"
    ...
    summary = {
        name: dict(
            trials=10,
            flagged=sum(r["unusual_sound"] for r in rows if r["case"] == name),
            pre_event_flags=sum(
                r["pre_event_flags"] for r in rows if r["case"] == name
            ),
        )
        for name in variants
    }
    result = dict(
        scope="synthetic quiet-room audio only",
        sample_rate=FS,
        threshold=model.threshold,
        training_seeds=[11, 12, 13, 14],
        calibration_seeds=[101, 102],
        test_seeds=list(range(1000, 1010)),
        results=summary,
        real_room_recordings=False,
