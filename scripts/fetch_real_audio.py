...
def get(path):
    dest=DATA/path;dest.parent.mkdir(parents=True,exist_ok=True)
    if not dest.exists():
...
def main():
    ...
    get('LICENSE');meta=get('meta/esc50.csv')
    ...
    for c in ['rain','engine']:
        for fold,n in [('1',3),('2',2)]:
            selected += [r for r in rows if r['category']==c and r['fold']==fold][:n]
    for c in ['rain','engine','footsteps','glass_breaking','door_wood_knock',
              'door_wood_creaks','thunderstorm','wind','clock_tick','coughing']:
        selected += [r for r in rows if r['category']==c and r['fold']=='5'][:3]
    manifest=[]
    def fetch(r):
        try:
            dest=get('audio/'+r['filename'])
            return {**r,'status':'downloaded','sha256':hashlib.sha256(dest.read_bytes()).hexdigest(),
                    'url':BASE+'audio/'+r['filename']}
        except Exception as e:return {**r,'status':'failed','error':str(e)}
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        for r in pool.map(fetch,selected):manifest.append(r)
    (DATA/'manifest.json').write_text(json.dumps(manifest,indent=2))
    print(json.dumps({'selected':len(selected),'downloaded':sum(r['status']=='downloaded' for r in manifest)}))
