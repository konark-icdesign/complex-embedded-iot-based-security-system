...
with urllib.request.urlopen(url,timeout=45) as r:archive.write_bytes(r.read())
with tarfile.open(archive) as tar:
    for member in tar.getmembers():
        if member.name=='arduino-cli':tar.extract(member,root,filter='data')
cli=str(root/'arduino-cli')
for command in [[cli,'version'],[cli,'core','update-index'],[cli,'core','install','arduino:renesas_uno']]:
