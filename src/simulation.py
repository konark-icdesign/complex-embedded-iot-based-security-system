...
class EvidenceBuffer:
    ...
    def push(self, t, audio, frame, row, trigger):
        ...
        if trigger and self.started is None:
            self.started = t
            self.saved = list(self.history)
        elif self.started is not None and not self.done:
            self.saved.append(packet)
        if self.started is not None and t - self.started >= 14.9:
            self.done = True
...
def run(case, model, seed=1000, config=None, capture=False):
    ...
    trace = []
    ai = 0
    last_cam = dict(motion=False, health=False, naive=0.0, fraction=0.0)
    fallback = False
    fallback_count = 0
    last_heartbeat = 0.0
    ...
    for k in range(240):
        ...
        if k % 2 == 0:
            ...
            if capture:
                frames.append(
                    np.zeros_like(base, dtype=np.uint8) if frame is None else frame
                )
                times.append(t)
        else:
            frame = None
        ...
        online = not (case.server_outage and 6.0 <= t < 19.0)
        usb = not (case.usb_outage and 6.0 <= t < 19.0)
        if online and usb:
            last_heartbeat = t
        degraded = t - last_heartbeat > 2.0
        ...
        e = dict(
            A=a,
            V=last_cam["motion"] if k % 2 == 0 else False,
            P=pp if usb else False,
            M=mm if usb else False,
            U=uu if usb else False,
        )
        ...
        row = dict(
            ...
            distance=float(filtered),
            motion_fraction=last_cam["fraction"],
    ...
    out = dict(
        name=case.name,
        group=case.group,
        intrusion=case.intrusion,
        seed=seed,
        red=bool(red_t),
        central_red=bool(fusion.alerts),
        local_fallback=fallback,
        yellow=any(v["state"] == 1 for v in trace),
    ...
    elif case.intrusion and not red_t:
        out["outcome"] = "MISSED_INTRUSION"
    else:
        out["outcome"] = "DETECTED" if case.intrusion else "NO_DANGER"
    extra = (
        dict(
            audio=x,
            features=f,
            audio_scores=scores,
            events=fusion.alerts,
            frames=frames,
            frame_times=times,
            evidence=buffer.saved,
        )
        if capture
        else {}
    )
