"""Ground truth belongs only to the generator/evaluator, never to detectors."""
...
def cases():
    ...
    specs=[
      Case('01_quiet'),Case('02_fan'),Case('03_traffic'),Case('04_sensor_noise'),
      Case('05_gain',gain=3.),Case('06_camera_noise'),
      ...
      Case('21_wifi_outage','fault',True,'steps','person','all',internet_outage=True),
      Case('22_server_offline','fault',True,'steps','person','all',server_outage=True),
      Case('23_walk_in','intrusion',True,'steps','person','all'),
      Case('24_quiet_entry','intrusion',True,'none','person','all'),
      Case('25_early_steps','intrusion',True,'steps','person','all',video_start=10.8),
      ...
      Case('31_audio_pir_only','intrusion',True,'steps','normal','p',note='Deliberately insufficient for RED'),
      Case('32_audio_radar_only','intrusion',True,'steps','normal','m',note='Deliberately insufficient for RED'),
      ...
      Case('36_delayed_camera','intrusion',True,'steps','person','p',video_start=11.2),
      Case('37_curtain','challenge',video='curtain'),
      Case('38_warm_moving_object','challenge',False,'none','warm','p',note='Common cause can fool vision+PIR'),
      ...
      Case('42_outside_door','challenge',False,'impact','normal','pm',note='Models bad sensor placement/cross-boundary response'),
      ...
      Case('49_usb_lost_intrusion','fault',True,'steps','normal','all',usb_outage=True),
      Case('50_rain_intrusion','intrusion',True,'soft_steps','person','all',context='rain'),
      Case('51_blind_silent_far','intrusion',True,'none','normal','m',note='Insufficient observability; intentional blind spot'),
      Case('52_stuck_radar','fault',physical='stuck_mm'),
    ]
    return specs

def waveform(case,seed,duration=24.):
    x=background(duration,seed,case.context,traffic=('traffic' in case.name))
    if case.audio=='silence':x[int(case.event_start*FS):]=0
    elif case.audio=='clipped':x=np.clip(x*1000,-1,1)
    else:x=add_event(x,case.audio,case.event_start,5.,seed+99)
...
def physical(case,t,rng):
    on=case.event_start<=t<14.
    p=False;m=False;d=3.+rng.normal(0,.02)
    mode=case.physical
    if on:
        if mode in ('all','p','pm','stationary'):p=True
        if mode in ('all','m','pm','stationary'):m=True
        if mode in ('all','u','stationary'):d=1.2+rng.normal(0,.025)
        if mode=='stationary' and t>9.5:p=False
    if mode=='echo_glitch' and 8.<=t<8.1:d=.35
    if mode=='pir_glitch' and 8.<=t<8.1:p=True
    if mode=='mm_glitch' and 8.<=t<8.1:m=True
    if mode=='timeout' and t>=8.:d=float('nan')
    if mode=='stuck_mm' and t>=8.:m=True
    return p,m,d
