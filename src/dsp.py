"""Audio features and a frozen median/MAD anomaly baseline."""
...
FEATURES = (
    ...
    "zcr",
    "log_crest",
    "subframe_energy_variation",
)
FLOORS = np.array([0.025, 0.025, 0.025, 0.025, 0.02, 0.08, 0.025, 0.10, 0.10])
...
def features(audio, fs=FS):
    ...
    clipped = np.mean(np.abs(frames) >= 0.999, axis=1) > 0.01
    ...
    crest = np.log(
        np.maximum(np.max(np.abs(frames), axis=1) / np.maximum(rms, 1e-15), 1)
    )
    ...
    return dict(
        ...
        x=feat,
        rms=rms,
        dbfs=20 * np.log10(np.maximum(rms, 1e-15)),
        valid=valid,
    )
...
class Baseline:
    ...
    def fit(cls, training, calibration):
        ...
        scale = np.maximum(
            1.4826 * np.median(np.abs(training - center), axis=0), FLOORS
        )
        model = cls(center, scale)
        model.threshold = max(
            3.5, float(np.quantile(model.score(calibration), 0.995) * 1.2)
        )
        return model
    ...
    def detect(self, f):
        ...
        persist = (
            np.convolve(raw.astype(int), np.ones(3, dtype=int), "full")[: len(raw)] >= 2
        )
        extreme = (
            (scores > 2 * self.threshold) & (f["x"][:, 7] > np.log(8)) & f["valid"]
        )
...
def background(seconds, seed, mode="dry", gain=1.0, traffic=False):
    ...
    low = lfilter([0.12], [1, -0.88], r.normal(size=len(t)))
    x = 0.003 * r.normal(size=len(t)) + 0.007 * low
    x += 0.005 * np.sin(2 * np.pi * 100 * t + phase) + 0.002 * np.sin(
        2 * np.pi * 240 * t + 0.3
    )
    x *= 1 + 0.08 * np.sin(2 * np.pi * 0.31 * t)
    if traffic:
        x += 0.006 * lfilter([0.04], [1, -0.96], r.normal(size=len(t)))
    if mode == "rain":
        white = r.normal(size=len(t))
        x += (
            0.02
            * lfilter([1, -0.7], [1], white)
            * (1 + 0.15 * np.sin(2 * np.pi * 0.7 * t))
        )
    return x * gain
...
def add_event(x, kind, start=8.0, duration=5.0, seed=0, amplitude=1.0):
    ...
    r = np.random.default_rng(seed)
    i = int(start * FS)
    length = min(int(duration * FS), len(y) - i)
    if length <= 0 or kind == "none":
        return y
    t = np.arange(length) / FS
    e = np.zeros(length)
    if kind in ("steps", "soft_steps"):
        for k in np.arange(0.0, duration, 0.58):
            u = t - k
            env = np.exp(-np.maximum(u, 0) * 22) * (u >= 0) * (u < 0.35)
            e += env * (
                0.075 * np.sin(2 * np.pi * 72 * u) + 0.018 * r.normal(size=length)
            )
        if kind == "soft_steps":
            e *= 0.12
        ...
        if kind == "thunder":
            noise = lfilter([0.1], [1, -0.9], noise)
        e = 0.22 * np.exp(-decay * t) * (0.6 * noise + 0.4 * np.sin(2 * np.pi * f * t))
    elif kind == "wind":
        e = (
            0.08
            * lfilter([0.025], [1, -0.975], r.normal(size=length))
            * (1 + np.sin(2 * np.pi * 0.6 * t))
        )
    elif kind == "speech":
        e = (
            0.02
            * (np.sin(2 * np.pi * 170 * t) + 0.5 * np.sin(2 * np.pi * 340 * t))
...
def train_synthetic():
    ...
    for mode in ("dry", "rain"):
        train = np.concatenate(
            [
                features(background(12, s, mode, g, traffic=True))["x"]
